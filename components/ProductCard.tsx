    ...
    <Card className="group relative overflow-hidden border-0 shadow-elegant hover:shadow-luxury hover-lift bg-white/80 backdrop-blur-sm">
        ...
        <Link to={`/product/${product.id}`}>
          <div className="aspect-square overflow-hidden bg-gradient-to-br from-souq-cream to-souq-sand">
            <img
              src={productImage}
              alt={product.name}
              className="h-full w-full object-contain transition-all duration-500 group-hover:scale-110 p-4 md:p-5"
            />
          </div>
        </Link>
      ...
      <CardContent className="p-4 md:p-4">
        <div className="space-y-3">
          <Badge variant="secondary" className="text-[10px] md:text-xs font-medium px-2.5 py-0.5 rounded-full bg-souq-gold-muted text-souq-navy">
            {product.category}
          </Badge>
          
          <Link to={`/product/${product.id}`}>
            <h3 className="font-semibold text-sm md:text-base hover:souq-text-gradient transition-all duration-300 line-clamp-2 leading-tight">
              {product.name}
            </h3>
          </Link>
          ...
          <div className="flex items-center justify-between pt-2">
            <div className="flex items-center gap-2">
              <span className="font-bold text-lg md:text-xl text-souq-gold">{product.price} L.E</span>
            ...
            <div className="text-[10px] md:text-xs text-muted-foreground bg-souq-sand px-2 py-1 rounded-md">
              Stock: {product.stock}
            </div>
          </div>
        </div>
